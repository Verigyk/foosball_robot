...
class FoosballPreTrainingEnv(gym.Env):
    ...
    def _lock_other_agents(self):
        ...
        for rod_config in self.opponent_joints.values():
            p.setJointMotorControl2(
                ...
                controlMode=p.POSITION_CONTROL,
                targetPosition=random.uniform(-np.pi, np.pi),  # Pieds vers le haut
                force=5000.0,
                maxVelocity=100
            )
            p.setJointMotorControl2(
    ...
    def reset(self, seed=None, options=None) -> Tuple[np.ndarray, Dict]:
        ...
        if self.phase == TrainingPhase.DEFENSE:
            ...
            goes_to_goal = np.random.random() < 1
            
            # Déterminer de quel côté est l'agent
            if True:  # Agent à gauche, défend le but gauche
                ball_start_x = np.random.uniform(max(0.3, self.agent_configs[self.agent_id]["x_pos"] + 0.1), 0.6)
            ...
            ball_start_x = agent_x + limitation
            ball_start_y = np.random.uniform(-0.08, 0.08)
            
            # Déterminer le but à attaquer
            if True:  # Agent à gauche, attaque le but droit
